...
site="site-${version#*-}"
# copy packages
for file in /mnt/$site/output/packages/modules/*; do
  version_name="${file##*/}"
done

if [ -n "$version_name" ]; then
    echo "copy packages $version_name"
    cp -r /mnt/$site/output/packages/modules/$version_name /var/www/packages/$version_name
else
...
rm -f /var/www/firmware/download/$version
